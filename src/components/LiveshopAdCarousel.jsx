...
function LiveshopAdCarousel({ data = carouselData, borderRadius = '18px' }) {
  // 🔁 Duplica os vídeos até ter pelo menos 10 slides
  const extendedData =
    data.length < 10
      ? Array.from({ length: Math.ceil(10 / data.length) }, () => data)
          .flat()
          .slice(0, 10)
      : data;
